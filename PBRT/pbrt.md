...
## 核心主线：从渲染方程出发，逐步构建完整渲染器

PBRT v3 的组织遵循一条清晰的主线：**提出问题（渲染方程）→ 分析被积函数（材质、光源、相机）→ 解决积分（蒙特卡洛、采样、光传）→ 扩展场景（体积渲染、双向方法）**。全书 27 章可分为六大模块，层层递进。
...
模块一：基础与系统 (Ch.1-4)
  ...
  │
  ▼
模块二：物理量与材质模型 (Ch.5,8,11)  ←── 渲染方程的被积函数 fᵣ
  │  回答"光是什么？表面如何与光交互？"
  │
  ▼
模块三：成像与采样 (Ch.6,7)  ←── 渲染方程的积分域与测量方式
  │  回答"光如何被记录？如何在积分域上采样？"
  │
  ▼
模块四：蒙特卡洛方法 (Ch.13)  ←── 渲染方程的数值解法
  │  回答"如何高效计算这个积分？"
  │
  ▼
模块五：光传算法 (Ch.14,15,16)  ←── 完整求解渲染方程
  │  回答"如何把前面所有部分组合起来，解出图像？"
  │
  ▼
模块六：进阶与回顾 (Ch.17-27)
  │  高级话题：GPU渲染、与处理、平面采样等
```

## 各模块详解

### 模块一：基础与系统 (Ch.1-4) — "搭建舞台"

| 章 | 标题 | 核心内容 | 与其他章的关系 |
|---|---|---|---|
| 1 | Introduction | 渲染历史、PBRT 设计哲学 | 全书导论 |
| 2 | Geometry and Transformations | 向量、法线、射线、变换矩阵 | 所有章节的数学基础 |
| 3 | Shapes | 球、圆柱、网格等几何体求交 | 光线求交的基础 |
| 4 | Primitives and Intersection Acceleration | BVH、K-d 树加速结构 | 光线求交的工程实现 |

**组织意图**：先搭好"场景表示 → 光线求交"的管线，后续章节才能讨论"求到交点后如何着色"。

---

### 模块二：物理量与材质模型 (Ch.5, 8, 11) — "定义被积函数"

这是全书的核心之一，回答"渲染方程中的 fᵣ 和 L 是什么"。

| 章 | 标题 | 核心内容 | 与其他章的关系 |
|---|---|---|---|
| **5** | **Color and Radiometry** | 辐射度量学（Φ, E, L, I）、光谱、颜色空间 | 定义了渲染方程中所有物理量的含义 |
| **8** | **Reflection Models** | BRDF/BTDF/BSSRDF 模型：Lambert、Microfacet (GGX/Beckmann)、Fresnel、Oren-Nayar | 渲染方程的 fᵣ 项——**表面**散射 |
| **11** | **Volume Scattering** | 吸收、发射、散射、相函数、透射率、BSSRDF 简化 | 渲染方程的 fᵣ 项——**体积**散射，是 Ch.8 的空间推广 |

**递进关系**：
```
Ch.5 (光是什么？辐射度量学)
  └→ Ch.8 (光在表面如何散射？BRDF/BTDF)
       └→ Ch.11 (光在介质内部如何散射？Phase Function/BSSRDF)
            └→ Ch.15 (如何求解体积散射的渲染方程？)
```

Ch.5 定义了"光的度量"，Ch.8 定义了"光在**表面**的行为"，Ch.11 将散射推广到**三维体积**。三者共同构成了渲染方程被积函数 fᵣ · L · cosθ 的全部物理含义。

---

### 模块三：成像与采样 (Ch.6, 7) — "如何观察与采样"

| 章 | 标题 | 核心内容 | 与其他章的关系 |
|---|---|---|---|
| **6** | **Camera Model** | 针孔相机、薄透镜、景深、弥散圆 | 定义了渲染方程的"测量方式"——从哪些光线出发 |
| **7** | **Sampling and Reconstruction** | 分层采样、Halton、Sobol、低差异序列、重构滤波 | 为 Ch.13 蒙特卡洛提供采样工具，为 Ch.14 路径追踪提供像素采样 |

**关系**：Ch.6 回答"渲染方程在哪积分"（相机光线方向），Ch.7 回答"如何在积分域上放置采样点"（采样策略）。两者共同决定了**积分的计算质量**。

---

### 模块四：蒙特卡洛方法 (Ch.13) — "如何数值求解积分"

| 章 | 标题 | 核心内容 | 与其他章的关系 |
|---|---|---|---|
| **13** | **Monte Carlo Integration** | MC 估计量、无偏性、重要性采样、MIS、俄罗斯轮盘赌、Metropolis 采样 | **全书的方法论核心**——连接渲染方程与实际计算 |

**关键地位**：渲染方程是一个高维积分，无法解析求解。Ch.13 提供了数值求解的全部数学工具：
- 基本估计量 → 路径追踪（Ch.14）
- 重要性采样 → 对 BRDF/光源的高效采样（Ch.14-16）
- MIS → 多种采样策略的组合（Ch.14-16）
- 俄罗斯轮盘赌 → 递归终止（Ch.14）
- Metropolis → MLT（Ch.16）

**Ch.7 与 Ch.13 的分工**：Ch.7 侧重**确定性**的低差异采样（准蒙特卡洛），Ch.13 侧重**随机性**的蒙特卡洛采样。两者在实践中结合使用。

---

### 模块五：光传算法 (Ch.14, 15, 16) — "组合一切，求解方程"

这是全书的**高潮**，将前述所有内容组装成完整的渲染算法。

| 章 | 标题 | 核心内容 | 前置依赖 |
|---|---|---|---|
| **14** | **Light Transport I: Surface Reflection** | 渲染方程的路径形式、Path Tracing、BDPT 雏形 | Ch.5(辐射度量) + Ch.8(BRDF) + Ch.13(MC) + Ch.7(采样) |
| **15** | **Light Transport II: Volume Rendering** | 传递方程、体积路径追踪、BSSRDF 的扩散近似 | Ch.11(体积散射) + Ch.14(光传框架) |
| **16** | **Light Transport III: Bidirectional Methods** | BDPT、Photon Mapping、MLT | Ch.14(路径追踪) + Ch.13(Metropolis) |

**递进关系**：
```
Ch.14 表面光传：基本 Path Tracing
  │  能处理大部分场景，但困难场景效率低
  │  （小光源、caustics、间接光照）
  ▼
Ch.15 体积光传：加入参与介质
  ...
  │  处理雾、云、皮肤（SSS）
  ▼
Ch.16 双向方法：从光源和相机两端出发
  │  BDPT：连接两条子路径
  │  Photon Mapping：光子→密度估计
  │  MLT：Metropolis 扰动路径
  │  解决 Ch.14 难以处理的场景
```
...
| 章节范围 | 内容 |
|---|---|
| Ch.17 | Retrospective and Future（回顾与展望） |
| Ch.18-19 | Advanced Topics（GPU 渲染等） |
| Ch.20-27 | 附录：与处理、平面采样、场景解析、debug 等 |
...
```
     ...
     被积函数     积分域      求解方法
        │           │           │
   ┌────┼────┐      │           │
   Ch.5 Ch.8 Ch.11  │        Ch.13 蒙特卡洛
   辐射量 表面 体积   │        (估计量/重要性采样/MIS/RR)
        │      │      │           │
        │      │      │        ┌──┼──┐
        │      │      │        │  │  │
        │      │      │      Ch14 Ch15 Ch16
...
1. **先读 Ch.1 + Ch.5**：建立全局概念，理解渲染方程的物理含义
2. **再读 Ch.8**：理解 BRDF 模型，这是 PBR 的核心
3. **然后读 Ch.13**：掌握蒙特卡洛方法，这是理解路径追踪的钥匙
4. **接着读 Ch.14**：Path Tracing 把前面所有内容串起来
5. **按需阅读**：Ch.11+Ch.15（体积渲染）、Ch.16（双向方法）、Ch.6（相机）、Ch.7（采样）
6. **Ch.2-4** 作为工程参考，按需查阅

> **核心洞察**：PBRT 的组织逻辑本质上是"**先定义问题（渲染方程），再分析问题的组成部分（被积函数、积分域），最后给出解决方案（蒙特卡洛 + 光传算法）**"。每一章都在为最终的 Path Tracing 积分器服务。

---

# 本文档涵盖的章节总结

> 以下为各章节的学习笔记。本文档涵盖 Ch.5, 6, 7, 8, 11, 12, 13, 14, 15, 16 的核心内容。
...
## 13.1 统计基础
参考[概率论](https://github.com/lingwei5/ProgrammingBasics/blob/master/mathamtics/statistics.md)

### 基本概念

| 术语 | 定义 | 数学表示 |
|---|---|---|
| **样本空间** | 试验的*所有*可能结果的集合 | Ω |
| **样本点** | 样本空间的基本元素/单个结果 | ω |
| **事件** | 样本空间的子集 | A ⊆ Ω |
| **随机变量** | 把每一个样本点映射到实数的一个函数，取值由概率决定的变量，分离散型和连续型 | X, Y, Z |
| **概率** | 事件发生的可能性，满足 0≤P(A)≤1 | P(A) |
| **独立** | 两个事件互不影响 | P(A∩B) = P(A)·P(B) |
...
相关系数：ρ = Cov[X,Y] / (σₓ · σᵧ),   ρ ∈ [-1, 1]
```

- ρ = 0：不相关（独立 ⟹ 不相关，但反过来不一定）
- ρ > 0：正相关；ρ < 0：负相关
...
| 大数定律 | N→∞ 时估计值收敛到真值 |
| 中心极限定理 | 误差分布近似高斯，收敛速度 O(1/√N) |
| 独立性 | 各采样点独立 ⟹ 方差可加 |
| Var[c]=0 | PDF ∝ f(x) 时方差为 0（重要性采样） |
...
| 符号 | 含义 | 数学本质 |
|---|---|---|
| **X（大写）** | 随机变量 | 一个**函数**：样本空间 Ω → ℝ，取值未确定 |
| **x（小写）** | 随机变量的取值（realization/观测值） | 一个**具体数值**，是 X 的一次"实现" |
...
按照某种分布pdf(x)采样随机变量X的意思是:  
进行N次采样得到了N个样本(就是N个实数)x1,x2,...xi...,xN  
如果N足够大，则对这N个样本xi画出统计直方图，则直方图符合pdf(x)的趋势(值不一定一样，但成比例,因为pdf是归一化后的结果，直方图可以不归一化)，表示为xi∼pdf(x)
...
> 本节内容整理自 [PBRT 3ed §13.4 Metropolis Sampling](https://www.pbr-book.org/3ed-2018/Monte_Carlo_Integration/Metropolis_Sampling)

### 13.4.1 Basic Algorithm（基本算法）

Metropolis 采样是一种从非负函数 f 中生成样本集的技术，样本的分布与 f 的值成正比。它的突出优势在于：**只需能计算 f 的值即可**，不必对 f 积分、归一化、再反演 CDF。而且**每次迭代都产生一个可用样本**，不像拒绝采样那样无法预估获得下一个样本所需的迭代次数。

它的主要缺点是：**相邻样本统计相关**，无法保证少量样本在定义域上均匀分布，只有在大量样本的极限下才覆盖整个定义域。因此分层采样等方差缩减技术（13.8）通常无法与 Metropolis 配合使用。
...
#### 5. 变异策略（Choosing Mutations）

变异策略有很大自由度，只要能计算转移密度 T 即可。对称变异甚至不需计算 T。多种策略可混合使用。
...
| 特性 | 说明 |
|------|------|
| **只需 f 的相对值** | 无需归一化常数，适合渲染中路径贡献 |
| **每次迭代必出样本** | 不像拒绝采样可能无限循环 |
| **样本相关** | 相邻样本统计相关，无法用分层采样等方差缩减 |
| **各态遍历性必需** | 必须保证能到达所有 f>0 的状态，需周期性大变异 |
| **启动偏差** | 需用 burn-in 或加权初始化解决 |
| **高维有效** | 在高维路径空间仍可工作，这是 MLT 的基础 |

---

- 如果pdf(x)积分易求，即CDF(x) = F(x)可知，
     - 如果cdf再可逆，则这个概率分布容易进行逆变换采样得到样本
     - 如果cdf不可逆，可以accept-reject采样
   - 如果pdf(x)积分难求，即CDF(x) = F(x)不可知，则可以metropolis-hastings采样,此时pdf(x)甚至都可以是未归一化的版本,因为归一化系数在比值中消掉了


### 13.4 附录：MCMC 数学原理与采样方法演进

本附录整理自以下资料，按 MCMC 理论发展顺序组织，作为 PBRT 13.4 的数学背景补充：

- [PBRT 3ed §13.4 Metropolis Sampling](https://www.pbr-book.org/3ed-2018/Monte_Carlo_Integration/Metropolis_Sampling)
- [MCMC(二)马尔科夫链 — 刘建平](https://www.cnblogs.com/pinard/p/6632399.html)
- [MCMC(三)MCMC采样和M-H采样 — 刘建平](https://www.cnblogs.com/pinard/p/6638955.html)
- [MCMC(四)Gibbs采样 — 刘建平](https://www.cnblogs.com/pinard/p/6645766.html)

#### 附录 A：马尔科夫链（Markov Chain）

> 参考：[MCMC(二)马尔科夫链](https://www.cnblogs.com/pinard/p/6632399.html)
...
利用马尔科夫链的收敛性：只要构造一个平稳分布为目标分布 π 的马尔科夫链，运行足够长后，链上的样本就近似服从 π。

**核心难题**：给定目标分布 π，如何找到对应的转移矩阵 P？这就是 MCMC 要解决的问题。

#### 附录 B：MCMC 采样与 M-H 采样

> 参考：[MCMC(三)MCMC采样和M-H采样](https://www.cnblogs.com/pinard/p/6638955.html)
...
> 参考：[MCMC(四)Gibbs采样](https://www.cnblogs.com/pinard/p/6645766.html)

##### C.1 重新寻找细致平稳条件

M-H 通过接受率使细致平稳条件成立。Gibbs 换一个思路：**利用条件概率分布直接构造满足细致平稳条件的转移矩阵**。
...
| 优势 | 说明 |
|------|------|
| **无需接受率** | 条件概率直接满足细致平稳条件，接受率恒为 1，无拒绝 |
| **高维高效** | 每步只对一个维度采样，计算量远低于 M-H |
| **只需条件分布** | 不需要联合分布，只需各维度的条件概率分布 |
| **大数据时代主流** | 高维数据中 MCMC 基本都用 Gibbs 采样 |

##### C.6 Gibbs 采样的限制

- **至少需二维**：一维分布无法用 Gibbs，此时仍用 M-H
- **需条件概率可采样**：各维度的条件概率分布必须能容易地采样
...
采样方法发展：
    ├─ 逆变换采样    （CDF 可逆时高效）
    ├─ 拒绝采样      （PDF 已知但 CDF 难求，独立样本）
    │
    └─ MCMC 方法族（基于马尔科夫链，样本相关）
           │
           ├─ MCMC 采样  （引入接受率 α，基础版）
           │      ↓ 接受率过低
           ├─ M-H 采样   （放大接受率，实用化）
           │      ↓ 高维计算困难 + 联合分布难求
           └─ Gibbs 采样 （条件概率分布，接受率=1，高维主流）
```

| 方法 | 接受率 | 样本独立性 | 高维表现 | 适用条件 |
|------|--------|-----------|---------|---------|
| 逆变换采样 | 100% | 独立 | 高维困难 | CDF 可逆 |
| 拒绝采样 | <100% | 独立 | 高维效率低 | 有包络分布 |
| MCMC | <100% | 相关 | 可工作 | 任意（仅需相对值） |
| M-H | 放大后较高 | 相关 | 计算量大 | 任意 + 提议分布 Q |
| **Gibbs** | **100%** | 相关 | **高效** | 各维度条件概率可采样 |

#### 附录 E：与 PBRT 渲染的关系

PBRT 13.4 的 Metropolis 采样是 MCMC 方法在渲染中的应用基础：

```
目标分布 f(x)：路径 x 的贡献函数（辐射度 × BRDF × 几何项）
提议分布 Q：  路径变异策略（如扰动光线方向、完全重新生成路径）
接受率 α：    基于 f 的比值（M-H 公式）
...
### Transforming between Distributions 这一节主旨
...
| 采样光源（面积/立体角） | 多变量雅可比 |
| Malley 方法 | 投影到半球 |
| Metropolis 采样中的 proposal 分布 | 变换理论 |

#### 一句话总结

> **这一节讲的是"如何通过对已知分布的样本做坐标变换，得到目标分布的样本"**——核心工具是**雅可比行列式**，它把原密度变换到新坐标下的密度。所有后续形状采样（球、半球、圆盘、三角形）都是这个理论的具体应用。

如果你在 [pbrt.md](file:///d:/mlw/doc/CGSummary/PBRT/pbrt.md#L1165) 第 1165 行研究 Metropolis-Hastings，这一节是它的前置知识——MH 的 proposal 分布 q(x→x') 经常就涉及分布变换，雅可比项在 MH 接受率里也会出现（若用非对称 proposal）。

需要我针对某个具体变换（如半球采样、Malley 法）展开推导吗？

## 13.6 2D Sampling with Multidimensional Transformations
讲了均匀采样 圆盘、余弦权重采样、圆锥、三角形、矩形、以及里面的坑
...
![alt text](通过圆盘均匀采样投影到半球得到余弦权重采样.png)
...
spliting：
      ...
      相当于光追时,一条光线往外扩散多条子光线而不是每条光线都从眼睛发出,也就是n个camera ray *m个shadow ray,比如为了对光源进行100次采样，则n=5,m=20,则总光线数为5+5*20=105次,而不是需要100+100=200次
...
> 本节梳理从基本渲染方程出发，经过四步推导，最终得到可被蒙特卡洛直接采样的路径积分形式的完整过程。
...
LTE（递归形式，Lₒ中含Lₒ，隐式）
    │  dω → dA·cosθ/r²
    ▼
LTE 面积形式 / 三点式（对场景表面积分）
    │  递归展开 k 次
    ▼
路径积分形式（显式多重积分，可直接MC采样）
    │  蒙特卡洛估计
    ▼
Path Tracing 算法
```
